...
exports.handler = async (event) => {
  ...
  const s3Client = new S3Client({ 
    ...
    requestHandler: new NodeHttpHandler({
      connectionTimeout: 15000, // Increased from 5000ms to 15000ms (15 seconds)
      socketTimeout: 90000,     // Increased from 60000ms to 90000ms (90 seconds)
      keepAlive: true,          // Enable TCP keepalive
    }),
    maxAttempts: 5,             // Retry up to 5 times
    retryMode: 'adaptive',      // Use adaptive retry mode for backoff
    endpoint: process.env.IS_LOCAL ? 'http://localhost:4566' : undefined, // Support local testing with LocalStack
  });
