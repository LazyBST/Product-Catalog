  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(url, { timeout: 30000 }, (response) => {
      ...
      if (response.statusCode === 301 || response.statusCode === 302) {
        ...
        console.log('[DEBUG] Following redirect to:', redirectUrl.toString());
        file.close();
        fs.unlinkSync(destPath); // Remove the file
        return downloadFile(redirectUrl.toString(), destPath)
          .then(resolve)
          .catch(reject);
      }
      ...
      fs.unlinkSync(destPath); // Remove the file
      reject(new Error('Connection timed out while downloading the file'));
    });
  });
}
 ...
 */
async function processBatchFile(client, batchFilePath, company_id, product_list_id) {
  console.log(`[DEBUG] Processing batch file: ${batchFilePath}`);
  
  // Read the file content
  const fileContent = fs.readFileSync(batchFilePath, 'utf8');
  const rows = fileContent.split('\n').filter(row => row.trim().length > 0);
  
  console.log(`[DEBUG] Read ${rows.length} rows from batch file`);
  
  // Process in batches of 1000 rows (to avoid query size limits)
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    batch.forEach(row => {
      const columns = parseCSVRow(row); // Function to properly parse CSV row
      
      if (columns.length < 6) return; // Skip invalid rows
      
      const [name, imageUrl, brand, barcode, companyIdStr, productListIdStr] = columns;
      ...
      // Add to values
      values.push(
        parseInt(companyIdStr, 10), // company_id
        name, // name
        imageUrl, // image_url
        brand, // brand
        barcode, // barcode
        parseInt(productListIdStr, 10), // product_list_id
        imageUrl ? (imageUrl.trim() !== '') : false, // has_image - properly handle null/undefined
        false, // is_ai_enriched
        Math.floor(Date.now() / 1000), // created_at (epoch)
        Math.floor(Date.now() / 1000)  // updated_at (epoch)
      );
    ...
    try {
      ...
      console.log(`[DEBUG] Inserted batch of ${placeholders.length} rows (${processed}/${rows.length})`);
    } catch (error) {
  ...
  console.log(`[DEBUG] Completed processing ${processed} rows from batch file`);
  return processed;
}
...
function parseCSVRow(row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if (char === '"') {
      // Toggle the inQuotes flag when we encounter quotes
      inQuotes = !inQuotes;
