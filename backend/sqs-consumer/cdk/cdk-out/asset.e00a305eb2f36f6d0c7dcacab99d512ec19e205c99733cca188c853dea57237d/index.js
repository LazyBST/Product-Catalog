...
async function processBatchFiles(pool, data) {
  ...
  for (let i = 0; i < batchedFilePaths.length; i++) {
    ...
    try {
      console.log(
        `[DEBUG] Processing batch file ${i + 1}/${
          batchedFilePaths.length
        }: ${batchFilePath}`
      );
      ...
      try {
        ...
        console.error(
          `[ERROR] Failed to get database connection for batch ${i + 1}:`,
          {
            error: connectionError.message,
            code: connectionError.code,
          }
        ...
        throw new Error(
          `Database connection failed for batch ${i + 1}: ${
      ...
      console.log(
        `[DEBUG] Batch ${i + 1} processed successfully. Updated metadata:`,
        updateResult.rows[0]
      );
    } catch (error) {
      ...
      if (client) {
        try {
          ...
          console.error(
            `[ERROR] Failed to rollback transaction for batch ${i + 1}:`,
            rollbackError.message
          );
        }
      }
      ...
      if (client) {
        try {
          ...
          console.error(
            `[ERROR] Failed to release client for batch ${i + 1}:`,
            releaseError.message
          );
        }
      }
  ...
  console.log(
    `[DEBUG] All ${batchedFilePaths.length} batches processed successfully`
  );
}
