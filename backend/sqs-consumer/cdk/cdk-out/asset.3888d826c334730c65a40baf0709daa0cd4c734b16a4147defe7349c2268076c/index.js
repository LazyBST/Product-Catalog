...
exports.handler = async (event) => {
  ...
  const s3Client = new S3Client({ 
    ...
    retryMode: 'adaptive'     // Use adaptive retry mode for backoff
  });
  const fs = require('fs');
            ...
            while (retries <= maxRetries) {
              try {
                if (retries > 0) {
                  ...
                  const delay = Math.floor(Math.random() * (1000 * Math.pow(2, retries)));
                ...
                // Only retry on network errors, not on permission issues
                if (!['TimeoutError', 'NetworkingError', 'ETIMEDOUT', 'ECONNRESET'].some(
                    errType => retryError.message.includes(errType)
                )) {
                  throw retryError; // Don't retry non-network errors
                }
              }
            }
