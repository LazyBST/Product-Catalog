...
exports.handler = async (event) => {
  ...
  // Replace AWS SDK imports with s3-client
  const s3Module = require('s3');
  const s3 = s3Module.createClient({
    maxAsyncS3: 20,     // this limits the number of concurrent S3 operations
    s3RetryCount: 5,    // number of retries for S3 operations
    s3RetryDelay: 1000, // delay in ms between retries
  ...
  // Remove the S3Client diagnostic test which uses AWS SDK v3
  try {
    console.log('[DEBUG] Testing S3 connectivity with s3-client...');
    
    // Simple test to list some objects
    const listParams = {
      s3Params: {
    ...
    const listObjects = s3.listObjects(listParams);
    
    listObjects.on('error', (err) => {
      console.error('[ERROR] Test S3 listing failed:', {
        error: err.message,
        code: err.code,
    ...
    listObjects.on('data', (data) => {
      if (data.Contents) {
        console.log('[DEBUG] S3 list test successful, found', data.Contents.length, 'objects');
      }
    });
      ...
      try {
        ...
        if (messageBody.Records && messageBody.Records[0] && messageBody.Records[0].s3) {
          ...
          try {
            ...
            // Use s3-client to download directly to a file
            await new Promise((resolve, reject) => {
              const downloader = s3.downloadFile({
              ...
              downloader.on('progress', () => {
                const progressData = downloader.progressAmount ? 
                  Math.floor((downloader.progressAmount / downloader.progressTotal) * 100) : 0;
                
                console.log(`[DEBUG] Download progress: ${progressData}%`);
              });
              
              // Handle errors
              downloader.on('error', (err) => {
                console.error('[ERROR] S3 download failed:', {
                  error: err.message,
                  code: err.code,
                  stack: err.stack?.split('\n').slice(0, 3).join('\n')
                });
            ...
            // No need for the pipeline step as the file is already downloaded
...
async function updateDatabase(pool, data) {
  ...
  try {
    // Validate file_status before sending to database
    // The enum only allows: 'uploaded', 'batched', 'processing', 'completed'
    if (data.file_status && !['uploaded', 'batched', 'processing', 'completed'].includes(data.file_status)) {
      console.warn(`[WARN] Invalid file_status value "${data.file_status}" detected, changing to "completed"`);
      data.file_status = 'completed';
    }
    ...
    const query = `
      ...
      ON CONFLICT ON CONSTRAINT product_list_meta_pkey DO UPDATE SET
