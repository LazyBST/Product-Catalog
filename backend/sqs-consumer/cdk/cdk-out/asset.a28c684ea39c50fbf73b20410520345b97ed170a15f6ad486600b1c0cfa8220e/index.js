...
exports.handler = async (event) => {
  ...
  // Replace s3-client with AWS SDK v3
  ...
  // Test direct S3 connection with simple operations
  try {
    console.log('[DEBUG] Testing S3 connectivity...');
    
    // Simple test to list a few objects
    try {
      ...
      console.error('[ERROR] S3 list test failed:', {
  ...
  try {
    ...
    pool = await createDatabasePool(Pool, dbConfig);
      ...
      try {
        ...
        if (messageBody.Records && messageBody.Records[0] && messageBody.Records[0].s3) {
          ...
          try {
            ...
            // Use AWS SDK v3 to download the file
            const getObjectResponse = await s3Client.send(new GetObjectCommand({
              Bucket: bucketName,
              Key: objectKey
            }));
            
            // Write the file to disk using streams
...
/**
 * Create a database connection pool - simplified version
 */
async function createDatabasePool(Pool, dbConfig) {
  try {
