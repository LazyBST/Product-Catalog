...
export class ExpressLambdaStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Resolve the absolute path to the Lambda package
    const lambdaPackagePath = path.resolve(__dirname, '../../../lambda-package');
    
    console.log(`Lambda package path: ${lambdaPackagePath}`);
    console.log(`Lambda package exists: ${fs.existsSync(lambdaPackagePath)}`);
    ...
    const expressLambda = new lambda.Function(this, 'ExpressLambdaFunction', {
      ...
      environment: {
        NODE_ENV: 'production',
        LAMBDA_EXECUTION: 'true', // Flag to indicate we're running in Lambda
        USE_AUTH_FALLBACK: 'true', // Force using our fallback auth
